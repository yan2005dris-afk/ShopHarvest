/**
 * Reader — Shein extension export (native NestJS, no bridge).
 *
...
/**
 * Read and validate the Shein extension export, persist it as-is to
 * `PIPELINE_RAW_DIR/shein/<capturedAt>.json`, and gate success on
...
export async function readSheinExtensionExport(
  ...
  const logger = new Logger('readSheinExtensionExport');
  ...
  const exportData = validateExtensionExport(parsed, 'shein');
  ...
  const errors = state === 'failed' ? ['shein: 0 items extracted'] : [];
...
if (require.main === module) {
  ...
  readSheinExtensionExport(
    { source: PipelineSource.SHEIN, outputDir: 'cli/shein' },
    configService,
  )
    .then((result) => {
      console.log(
        `shein: items=${result.totalScraped} outputPath=${result.outputPath}`,
      );
      process.exit(result.totalScraped > 0 ? 0 : 1);
    })
    .catch((err) => {
      console.error('shein: fatal', err);
      process.exit(1);
    });
}
