...
@Component({
  ...
  styles: [`
    ...
    .vm-status-title { margin: 0; font-size: 1.125rem; font-weight: 600; color: var(--color-on-surface); }
    .vm-status-sub { margin: 0; font-size: 0.875rem; color: var(--color-on-surface-variant); line-height: 1.5; }
    .vm-pulse { position: relative; width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; }
    .vm-pulse-ring { position: absolute; width: 52px; height: 52px; border-radius: 50%; background: color-mix(in srgb, var(--color-primary) 15%, transparent); animation: pulse-ring 1.5s ease-out infinite; }
    .vm-pulse-dot { width: 16px; height: 16px; border-radius: 50%; background: var(--color-primary); }
