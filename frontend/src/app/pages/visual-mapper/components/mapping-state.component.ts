...
@Component({
  ...
  styles: [`
    ...
    .vm-sidebar { width: 280px; min-width: 280px; background: var(--surface); border-right: 1px solid var(--border); display: flex; flex-direction: column; overflow: hidden; }
    .vm-sidebar-top { padding: 1.25rem 1.25rem 0.75rem; border-bottom: 1px solid var(--border); }
    .vm-sidebar-title { margin: 0 0 0.25rem; font-size: 1rem; font-weight: 700; color: var(--text-1); }
    .vm-page-label { margin: 0 0 0.75rem; font-size: 0.75rem; color: var(--text-2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .vm-progress-bar-track { height: 3px; background: var(--surface-3); border-radius: 999px; margin-bottom: 0.375rem; }
    .vm-progress-bar-fill { height: 100%; background: var(--accent); border-radius: 999px; transition: width 0.3s ease; }
    .vm-progress-text { margin: 0; font-size: 0.75rem; color: var(--text-3); }
    .vm-fields-list { flex: 1; overflow-y: auto; padding: 0.75rem; display: flex; flex-direction: column; gap: 0.375rem; }
    .vm-field { padding: 0.625rem 0.75rem; border-radius: var(--radius); border: 1px solid var(--border); background: var(--surface-2); }
    .vm-field--ok { border-color: var(--success-border); background: var(--success-dim); }
    .vm-field-row { display: flex; align-items: center; gap: 0.5rem; }
    .vm-field-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--surface-3); flex-shrink: 0; }
    .dot--ok { background: var(--success); }
    .vm-field-name { font-size: 0.8125rem; font-weight: 600; color: var(--text-1); flex: 1; }
    .vm-remove-btn { display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; border: none; background: var(--danger-dim); color: var(--danger); border-radius: 50%; font-size: 0.875rem; cursor: pointer; padding: 0; line-height: 1; flex-shrink: 0; }
    .vm-remove-btn:hover { background: var(--danger); color: #fff; }
    .vm-selector { margin-top: 0.375rem; }
    .vm-selector-text { display: block; font-family: var(--font-mono); font-size: 0.6875rem; color: var(--text-2); background: var(--surface); padding: 0.25rem 0.5rem; border-radius: 0.25rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .vm-pending { margin-top: 0.25rem; font-size: 0.6875rem; color: var(--text-3); font-style: italic; }
    .vm-container-block { padding: 0.75rem; border-top: 1px solid var(--border); }
    .vm-section-label { margin: 0 0 0.5rem; font-size: 0.6875rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.07em; color: var(--text-3); }
    .vm-preview { flex: 1; background: var(--bg); display: flex; overflow: hidden; }
  `],
})
